...
function schulte(element, matchstype, days) {
    ...
    for (let i = 1; i <= element.length / 2; i++) {
        ...
        const match =
        {
            ...
            opposermatchplayed: (element[a]).noOfMatchs + 1
        }
    ...
    for (i = 1; i <= list.length; i++) {
        ...
        if (winningresult[i - 1] == "win") {
            const winner = {
                ...
                noOfMatchs: list[i - 1].challengermatchplayed
            }
            ...
            const winner = {
                ...
                noOfMatchs: list[i - 1].opposermatchplayed
            }
            resultOfMatch.push(winner)
        }
    }
    console.log("-----------winner list------------");
    len = schultteling.length
...
schulte(accessingObject, matchtype, accesingdate)